...
depends_on:
  ...
  - artifact: "shipglowz_data/workflow/specs/daily-mail-intake-review-v2.md"
    artifact_version: "1.0.0"
...
- Keep secrets and OAuth tokens outside `~/.shipglowz/private/data/` and outside the repository.
...
- Existing daily review spec:
  - `shipglowz_data/workflow/specs/daily-mail-intake-review-v2.md`
...
- [ ] Task 2: Choose storage and command surface
  ...
  - Action: Implement the decided shape: declarative registry rooted at `~/.shipglowz/private/data/mail-admin/` plus sibling CLI `scripts/mail-admin`.
...
None. This spec is ready on the following decisions: rules live in `~/.shipglowz/private/data/mail-admin/`, the command surface is `scripts/mail-admin`, and the supported destructive shortcut is immediate Gmail Trash rather than archive-by-default.
