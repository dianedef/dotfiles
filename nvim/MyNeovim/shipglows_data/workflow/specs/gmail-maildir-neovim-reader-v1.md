...
evidence:
  ...
  - "lua/shipglows/init.lua already hosts local Neovim commands and mappings."
...
- `notmuch` setup and indexing for the local Maildir.
  - A centralized local plugin module under `lua/shipglows/mail/`.
- A local CLI, tentatively `mail-intel`, with read-only commands:
  ...
  - `show`
  - `export --markdown`
  - A minimal Neovim setup hook from `lua/shipglows/init.lua` into the centralized mail module, exposing commands:
  - `:CompetitorMailInbox`
...
- Do not persist email contents in ShipGlows specs, docs, logs, or examples.
...
- Keep the feature implementation centralized under `lua/shipglows/mail/`; the only allowed touchpoint in existing Neovim config is a small setup call/command registration from `lua/shipglows/init.lua`.
...
- Existing `lua/shipglows/init.lua` command registration pattern.
...
- `lua/shipglows/init.lua`: must remain a thin setup/delegation point, not the main implementation.
- `lua/shipglows/mail/`: dedicated implementation directory for config, CLI calls, rendering, and command handlers.
...
- [x] Task 1: Add centralized mail module configuration contract
  - File: `lua/shipglows/mail/config.lua`
  ...
  - Depends on: none
  - Validate with: `nvim --headless "+lua require('shipglows.mail.config')" +qa`
...
- [x] Task 4: Add Neovim mail module
  ...
  - Validate with: `nvim --headless "+lua require('shipglows.mail')" +qa`
...
- [x] Task 4.1: Split mail implementation into small files only if needed
  - File: `lua/shipglows/mail/*.lua`
  - Action: Keep parsing of CLI results, buffer rendering, picker/list handling, and copy behavior in the `lua/shipglows/mail/` directory if the implementation grows.
  - User story link: Keeps the plugin maintainable without scattering behavior across the dotfiles config.
  - Depends on: Task 4
  - Validate with: `rg -n "CompetitorMail|mail%-intel|notmuch" lua/config lua/plugins lua/shipglows`
...
- [x] Task 5: Register Neovim commands and mappings
  - File: `lua/shipglows/init.lua`
  - Action: Delegate to `require("shipglows.mail").setup()` for `CompetitorMailInbox`, `CompetitorMailSearch`, `CompetitorMailOpen`, and `CompetitorMailCopyMarkdown`; add optional leader mappings only if they can stay inside the mail module setup.
  - User story link: Gives the user direct commands for reading and copying selected emails.
  - Depends on: Task 4
  - Validate with: `nvim --headless "+lua require('shipglows').setup()" +qa`
...
| 2026-05-17 14:38:34 UTC | sf-spec | GPT-5 Codex | Clarified implementation layout after user asked whether code must be scattered through the config | Spec now requires a centralized `lua/shipglows/mail/` module with only a thin setup hook in `lua/shipglows/init.lua` | /sf-ready gmail maildir neovim reader v1 |
