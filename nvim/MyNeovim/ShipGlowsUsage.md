...
ShipGlows is a NeoVim helper module that allows users to navigate between Markdown headings and control the shared explorer panel width with compact shortcuts.
...
It is loaded from `lua/config/keymaps.lua` with `require("shipglows").setup()`.

## Usage

Once loaded, ShipGlows provides the following default shortcuts for navigating between headers:
...
- **Panel size 1**: `:ShipGlowsPanel1` or `<leader>w1`
- **Panel size 2**: `:ShipGlowsPanel2` or `<leader>w2`
- **Panel size 3**: `:ShipGlowsPanel3` or `<leader>w3`
- **Panel full size**: `:ShipGlowsPanelFull` or `<leader>wF`
...
You can customize the shortcuts by modifying `lua/shipglows/init.lua`.
