...
- `lua/shipflow/init.lua` - A local Lua module already groups custom commands and mappings.
...
- Summary: Extract existing custom helpers into a first-class local plugin, likely under `lua/shipflow/`.
...
Start with a local Neovim "workflow cockpit" plugin under `lua/shipflow/`, backed by small testable Lua functions and user commands. Keep the boundary clean enough that selected actions can later call a CLI or MCP server.
