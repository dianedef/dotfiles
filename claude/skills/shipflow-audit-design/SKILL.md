...
1. Read `/home/claude/ShipFlow/PROJECTS.md` — check the **Domain Applicability** table. Identify projects with ✓ in the Design column.
...
5. Update `/home/claude/AUDIT_LOG.md` (one row per project, Design column) and `/home/claude/TASKS.md` (each project's `### Audit: Design` subsection).
...
- Options:
  ...
  - One option per UI project from `/home/claude/ShipFlow/PROJECTS.md`: label = project name, description = stack
...
1. **Global `/home/claude/AUDIT_LOG.md`**: append a row filling only the Design column, `—` for others.
