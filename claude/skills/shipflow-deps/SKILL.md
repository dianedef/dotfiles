...
1. Read `/home/claude/ShipFlow/PROJECTS.md` — check the **Domain Applicability** table. Identify projects with ✓ in the Deps column. BuildFlowz (Bash) has no package manager → skip.
...
5. Update `/home/claude/AUDIT_LOG.md` (one row per project, Deps column) and `/home/claude/TASKS.md` (each project's `### Audit: Deps` subsection).
...
- Options:
  ...
  - One option per project from `/home/claude/ShipFlow/PROJECTS.md`: label = project name, description = stack
...
1. **Global `/home/claude/AUDIT_LOG.md`**: append a row filling the Deps column, `—` for other domains.
...
2. **Master `/home/claude/TASKS.md`**: find the project's section, add/replace a `### Audit: Deps` subsection with the same tasks.
