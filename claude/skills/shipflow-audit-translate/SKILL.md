...
1. Read `/home/claude/ShipFlow/PROJECTS.md` — check the **Domain Applicability** table. Identify projects with ✓ in the Translate column.
...
5. Update `/home/claude/AUDIT_LOG.md` (one row per project, Translate column) and `/home/claude/TASKS.md` (each project's `### Audit: Translate` subsection).
...
- Options:
  ...
  - One option per multilingual project from `/home/claude/ShipFlow/PROJECTS.md`: label = project name, description = stack
...
1. **Global `/home/claude/AUDIT_LOG.md`**: append a row filling only the Translate column, `—` for others.
...
2. **Master `/home/claude/TASKS.md`**: find the project's section, add/replace an `### Audit: Translate` subsection with the same tasks.
