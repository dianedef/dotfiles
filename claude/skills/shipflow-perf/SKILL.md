...
1. Read `/home/claude/ShipFlow/PROJECTS.md` — check the **Domain Applicability** table. Identify projects with ✓ in the Perf column.
...
5. Update `/home/claude/AUDIT_LOG.md` (one row per project, Perf column) and `/home/claude/TASKS.md` (each project's `### Audit: Perf` subsection).
...
1. **Global `/home/claude/AUDIT_LOG.md`**: append a row filling the Perf column, `—` for other domains.
...
2. **Master `/home/claude/TASKS.md`**: find the project's section, add/replace a `### Audit: Perf` subsection with the same tasks.
