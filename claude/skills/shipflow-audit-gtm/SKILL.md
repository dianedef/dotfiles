...
1. Read `/home/claude/ShipFlow/PROJECTS.md` — check the **Domain Applicability** table. Identify projects with ✓ in the GTM column.
...
5. Update `/home/claude/AUDIT_LOG.md` (one row per project, GTM column) and `/home/claude/TASKS.md` (each project's `### Audit: GTM` subsection).
...
- Options:
  ...
  - One option per commercial project from `/home/claude/ShipFlow/PROJECTS.md`: label = project name, description = stack
...
1. **Global `/home/claude/AUDIT_LOG.md`**: append a row filling only the GTM column, `—` for others.
