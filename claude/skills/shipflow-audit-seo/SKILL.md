...
1. Read `/home/claude/ShipFlow/PROJECTS.md` — check the **Domain Applicability** table. Identify projects with ✓ in the SEO column.
...
5. Update `/home/claude/AUDIT_LOG.md` (one row per project, SEO column) and `/home/claude/TASKS.md` (each project's `### Audit: SEO` subsection).
...
- Options:
  ...
  - One option per SEO-applicable project from `/home/claude/ShipFlow/PROJECTS.md`: label = project name, description = stack
...
1. **Global `/home/claude/AUDIT_LOG.md`**: append a row filling only the SEO column, `—` for others.
...
2. **Master `/home/claude/TASKS.md`**: find the project's section, add/replace an `### Audit: SEO` subsection with the same tasks. Update the Dashboard "Top Priority" if critical issues found.
