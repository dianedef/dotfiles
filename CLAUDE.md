...
Canonical engines are `install-dotfiles.ps1` and `dotfiles/install-dotfiles.sh`; both consume `dotfiles/components.tsv`.
The public Unix bootstrap is `install-dotfiles.sh`, which ensures a clean validated checkout at
`$DOTFILES_DIR` before delegating to `dotfiles/install-dotfiles.sh`. Compatibility entrypoints remain thin shims.

Keep Termux independent and unchanged unless explicitly scoped.
