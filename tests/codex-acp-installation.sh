...
awk -F '\t' '$1=="windows-native-toolchain"{a=($11=="BrechtSanders.WinLibs.POSIX.UCRT"&&$19=="gcc.exe&cmake.exe&ninja.exe")}$1=="tree-sitter-cli"{b=($11=="tree-sitter.tree-sitter-cli"&&$19=="tree-sitter.exe")}$1=="codex-acp"{c=($5=="neovim,windows-native-toolchain,tree-sitter-cli")}END{if(!a||!b||!c)exit 1}' "$M"
...
rg -n 'Test-HealthProbe|split.*&' "$P" >/dev/null
