...
- Couche installation:
  ...
  - Le namespace canonique est exclusivement `shipglows`: données gouvernées sous `shipglows_data/`, modules Neovim sous `lua/shipglows/`, variables `SHIPGLOWS_*` et état privé sous `~/.shipglows/`. Aucun alias de l'ancien namespace n'est conservé.
...
- Le panneau explorateur Neovim partage sa largeur via `nvim/MyNeovim/lua/config/explorer-panel.lua`; `shipglows` expose `ShipGlowsExplorerWidth20`, `ShipGlowsExplorerWidth35` et `ShipGlowsExplorerWidthFull` avec les mappings `<leader>e2`, `<leader>e3`, `<leader>eF` pour Snacks et Neo-tree.
